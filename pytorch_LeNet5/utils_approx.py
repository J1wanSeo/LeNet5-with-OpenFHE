...
def poly_eval(x, coeff):
    coeff = torch.tensor(coeff).to(x.device)
    if len(x.size()) == 2:
        return torch.sum(x[:,:,None]**torch.arange(coeff.size(0)).to(x.device)[None,None,:]*coeff, dim=-1)
    elif len(x.size()) == 4:
        return torch.sum(x[:,:,:,:,None]**torch.arange(coeff.size(0)).to(x.device)[None,None,None,None,:]*coeff, dim=-1)
