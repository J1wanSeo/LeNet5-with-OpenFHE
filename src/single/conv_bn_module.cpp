...
std::vector<int> GenerateUnifiedConvRotationIndices(
    ...
    for (size_t ch = 0; ch < in_channels; ++ch) {
        size_t base = ch * inputW * inputW;
        for (size_t dy = 0; dy < filterH; ++dy) {
            for (size_t dx = 0; dx < filterW; ++dx) {
                rotSet.insert(base + dy * inputW + dx);
            }
        }
    }
    return std::vector<int>(rotSet.begin(), rotSet.end());
...
Ciphertext<DCRTPoly> UnifiedConvBnLayer(
    ...
    std::vector<Ciphertext<DCRTPoly>> ct_outputs(out_channels);
    ...
    for (size_t out_ch = 0; out_ch < out_channels; ++out_ch) {
        ...
        for (size_t in_ch = 0; in_ch < in_channels; ++in_ch) {
            ...
            for (size_t dy = 0; dy < filterH; ++dy) {
                for (size_t dx = 0; dx < filterW; ++dx) {
                    int shift = in_ch * inputH * inputW + dy * inputW + dx; // input size + dy*input + dx
                    auto ct_rot = cc->EvalRotate(ct_all_input, shift);

                    // 마스크: 해당 채널의 출력 영역
                    std::vector<double> mask(slotCount, 0.0);
                    for (size_t i = 0; i < outH; ++i) {
                        for (size_t j = 0; j < outW; ++j) {
                            size_t slot = out_ch * outH * outW + i * outW + j;
                            mask[slot] = 1.0;
                        }
                    }
                    auto pt_mask = cc->MakeCKKSPackedPlaintext(mask);

                    auto masked = cc->EvalMult(ct_rot, pt_mask);
                    masked = cc->Rescale(masked);

                    auto weighted = cc->EvalMult(masked, filter[dy * filterW + dx]);
                    weighted = cc->Rescale(weighted);
                    partials.push_back(weighted);
                }
            }
        }
        ct_outputs[out_ch] = cc->EvalAddMany(partials);
    }

    Ciphertext<DCRTPoly> ct_merged = cc->EvalAddMany(ct_outputs);
    return ApplyBiasBN_AllPacked(cc, ct_merged, biases, gammas, betas, means, vars, outH, outW, out_channels);
}

Ciphertext<DCRTPoly> ApplyBiasBN_AllPacked( // pakcing bn coefficients to fit output size
    CryptoContext<DCRTPoly> cc,
    ...
    std::vector<double> scale(slotCount, 0.0);
    std::vector<double> shift(slotCount, 0.0);

    for (size_t ch = 0; ch < out_channels; ++ch) {
        double a = gammas[ch] / std::sqrt(vars[ch] + eps);
        double b = betas[ch] - a * means[ch] + biases[ch];

        for (size_t i = 0; i < outH * outW; ++i) {
            size_t idx = ch * outH * outW + i;
            if (idx < slotCount) {
                scale[idx] = a;
                shift[idx] = b;
            }
    ...
    auto pt_a = cc->MakeCKKSPackedPlaintext(scale);
    auto pt_b = cc->MakeCKKSPackedPlaintext(shift);

    auto ct_scaled = cc->EvalMult(ct, pt_a);
    ct_scaled = cc->Rescale(ct_scaled);
...
void SaveDecryptedPackedOutput(
    ...
    Plaintext pt;
    cc->Decrypt(sk, ct, &pt);
    pt->SetLength(outH * outW * out_channels);
    auto vec = pt->GetRealPackedValue();
...
Ciphertext<DCRTPoly> UnifiedAvgPool2x2(
    ...
    for (size_t dy = 0; dy < filterH; ++dy) {
        for (size_t dx = 0; dx < filterW; ++dx) {
            ...
            auto weighted = cc->EvalMult(rotated, coeff);
            weighted = cc->Rescale(weighted);
            partials.push_back(weighted);
        }
    }

    Ciphertext<DCRTPoly> ct_sum = cc->EvalAddMany(partials);

    std::vector<double> mask(slotCount, 0.0);
    for (size_t ch = 0; ch < out_channels; ++ch) {
        size_t base_in = ch * inputH * inputW;
        size_t base_out = ch * outH * outW;

        for (size_t i = 0; i < outH; ++i) {
            for (size_t j = 0; j < outW; ++j) {
                size_t input_idx = base_in + (i * stride * inputW) + (j * stride);
                size_t output_idx = base_out + i * outW + j;
                if (input_idx < slotCount && output_idx < slotCount) {
                    mask[input_idx] = 1.0;
                }
            }
        }
    ...
    auto pt_mask = cc->MakeCKKSPackedPlaintext(mask);
        auto result = cc->EvalMult(ct_sum, pt_mask);
