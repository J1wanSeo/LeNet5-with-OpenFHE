...
Ciphertext<DCRTPoly> GeneralConv2D_CKKS(
        ...
        for (size_t dx = 0; dx < filterW; dx++) {
            ...
            for (size_t i = 0; i < outH; i++) {
                for (size_t j = 0; j < outW; j++) {
                    size_t input_row = i * stride + dy;
                    size_t input_col = j * stride + dx; 
                    if (input_row < inputH && input_col < inputW) {
                        size_t output_slot = i * outW + j;  // compact 저장
    ...
    // 디버깅용 코드
    std::cout << "=== Convolution 마스킹 위치 ===" << std::endl;
    for (size_t i = 0; i < outH; i++) {
        for (size_t j = 0; j < outW; j++) {
            size_t conv_idx = i * outW + j;;
            size_t bias_idx = i * outW + j;;
            std::cout << "Output(" << i << "," << j << "): conv_slot=" 
                    << conv_idx << ", bias_slot=" << bias_idx << std::endl;
        }
    }
